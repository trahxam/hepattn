...
def plot_trackml_event_reconstruction(inputs, reconstruction, x_coord="x", y_coord="y"):
    ...
    ax.scatter(inputs[f"hit_{x_coord}"][batch_idx][hit_particle_valid],
    ...
    for i in range(reconstruction["particle_valid"][batch_idx].shape[-1]):
        ...
        ax.plot(inputs[f"hit_{x_coord}"][batch_idx][particle_mask],
                inputs[f"hit_{y_coord}"][batch_idx][particle_mask], alpha=0.5)
