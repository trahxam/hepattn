# ruff: noqa: N803,N806,SIM108,RET504
"""Track visualization utilities for ROOT-based muon tracking data.

This module provides the TrackVisualizerMDTGeometry class for visualizing
ATLAS muon spectrometer tracks in three 2D projections using MDT geometry
representation (wire endpoints) from ROOT files.
"""
...
class TrackVisualizerMDTGeometry:
    """Visualizer for ATLAS muon tracks from ROOT files using MDT geometry.
    ...
    def plot_muon_tracks(self, event_index: int, show_plot: bool = True, just_background: bool = False) -> plt.Figure | None:
        ...
        try:
            with uproot.open(self.root_file_path) as file:
                ...
                all_truth = tree["spacePoint_truthLink"].array(library="np")[event_index]
                ...
                print(f"Event {event_number} (index {event_index}): Found {len(all_high_x)} space points")
    ...
    def plot_and_save_event(
        ...
        if fig_background is not None and save_path is not None:
            save_path_bg = save_path.replace(".png", "_background.png")
