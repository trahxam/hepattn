...
from hepattn.experiments.tide.data import ROIDataset
...
def main():
    ...
    with h5py.File(eval_path) as file:
        for i, sample_id in tqdm(enumerate(file.keys())):
            ...
            preds["pred_valid"] = sigmoid(outputs[f"pred_valid"]["pred_logit"][0]) >= 0.5
            preds["pred_pix_valid"] = sigmoid(outputs[f"pred_pix_assignment"]["pred_pix_logit"][0]) >= 0.5
            preds["pred_sct_valid"] = sigmoid(outputs[f"pred_sct_assignment"]["pred_sct_logit"][0]) >= 0.5
