...
OUTPUT_PATH = Path("src/hepattn/experiments/colliderml/plots/event_phi_index_span.png")
EVENT_INDEX = 0
EVENT_TYPE = "ttbar"
...
def get_hit_phi_and_associations(inputs, targets):
    if HIT_COLLECTION == "tracker":
        phi = inputs["sihit_phi"].cpu().numpy()
    ...
    return phi, hit_valid, association_indptr, association_indices, label


def compute_particle_phi_spans(phi, hit_valid, association_indptr, association_indices, particle_valid):
    valid_hit_indices = np.flatnonzero(hit_valid)
    if valid_hit_indices.size == 0:
        return np.zeros(0, dtype=np.int64), 0

    # Map each hit index to its rank after sorting hits by phi.
    phi_sorted_hit_indices = valid_hit_indices[np.argsort(phi[valid_hit_indices], kind="mergesort")]
    phi_rank = np.full(phi.shape[0], -1, dtype=np.int64)
    phi_rank[phi_sorted_hit_indices] = np.arange(phi_sorted_hit_indices.size, dtype=np.int64)
    ...
    for particle_idx in range(num_particles):
        ...
        hit_indices = hit_indices[(hit_indices >= 0) & (hit_indices < phi.shape[0])]
        if hit_indices.size == 0:
            continue

        ranks = phi_rank[hit_indices]
        ranks = ranks[ranks >= 0]
        if ranks.size == 0:
            continue
    ...
    return np.asarray(spans, dtype=np.int64), int(phi_sorted_hit_indices.size)
...
def plot_hist_and_cdf(spans, sample_id, hit_label, num_valid_hits):
    ...
    ax_hist.set_xlabel("Phi-ordered index span (max_idx - min_idx)")
    ...
    ax_cdf.set_xlabel("Phi-ordered index span (max_idx - min_idx)")
...
def main():
    ...
    phi, hit_valid, association_indptr, association_indices, hit_label = get_hit_phi_and_associations(inputs, targets)
    ...
    for subset_name, subset_valid_mask, subset_label in subsets:
        spans, num_valid_hits = compute_particle_phi_spans(
            phi=phi,
        ...
        plt.close(fig)
        num_saved += 1
        print(f"Saved phi index span histogram/CDF ({subset_name}) to: {output_path.resolve()}")
