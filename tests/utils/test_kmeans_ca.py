...
def test_requires_k_or_logits():
    ...
    q = torch.zeros(1, 1, 2)
    v = torch.zeros(1, 1, 2)
    with pytest.raises(ValueError, match="Provide either affinity_logits or k"):
        module(q, v=v)
...
def test_q_mask_forces_single_query():
    ...
    affinity_logits = torch.tensor([[[1.0, 1.0], [2.0, 2.0]]])
    q_mask = torch.tensor([[True, False]])
    out = module(q, v=v, affinity_logits=affinity_logits, q_mask=q_mask)
...
def test_kv_mask_excludes_key():
    ...
    q = torch.zeros(1, 2, 2)
    v = torch.tensor([[[1.0, 1.0], [2.0, 2.0]]])
    affinity_logits = torch.tensor([[[1.0, 1.0], [0.0, 0.0]]])
    kv_mask = torch.tensor([[True, False]])
    out = module(q, v=v, affinity_logits=affinity_logits, kv_mask=kv_mask)
...
def test_attn_mask_respected():
    ...
    affinity_logits = torch.tensor([[[0.0], [10.0]]])
    attn_mask = torch.tensor([[[True], [False]]])
    out = module(q, v=v, affinity_logits=affinity_logits, attn_mask=attn_mask)
