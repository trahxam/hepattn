...
def test_encoderlayer_with_kwargs(input_tensor):
    dim = input_tensor.shape[-1]
    model = EncoderLayer(dim=dim, drop_path=0.1, attn_kwargs={"num_heads": 4}, window_size=10)
    output = model(input_tensor)
    assert output.shape == input_tensor.shape
...
def test_encoder_with_dict_inputs():
    batch_size = 8
    seq_len = 16
    dim = 64
    ...
    model = Encoder(num_layers=2, dim=dim)
    output, _ = model(x)
...
def test_encoder_empty_input():
    model = Encoder(num_layers=1, dim=64)
