...
def test_plot_cld_hit_coords(cld_datamodule):
    ...
    for i in range(len(targets["particle_valid"])):
        ...
        particle_min_abs_dphi = torch.min(particle_abs_dphi, dim=-1).values
        particle_min_abs_deta = torch.min(particle_abs_deta, dim=-1).values
        particle_min_dR = torch.min(particle_dR, dim=-1).values
    ...
    ax[0].hist2d(torch.cat(pts), torch.cat(dphis), bins=(np.geomspace(0.1, 100.0, 32), np.geomspace(1.0, 1000*np.pi, 32)), norm=LogNorm(vmin=1, vmax=None))
    ax[1].hist2d(torch.cat(pts), torch.cat(detas), bins=(np.geomspace(0.1, 100.0, 32), np.geomspace(1.0, 1000*np.pi, 32)), norm=LogNorm(vmin=1, vmax=None))
    ax[2].hist2d(torch.cat(pts), torch.cat(dRs), bins=(np.geomspace(0.1, 100.0, 32), np.geomspace(0.1, 1000*np.pi, 32)), norm=LogNorm(vmin=1, vmax=None))
